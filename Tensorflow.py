...
class Tensorflow:
    def __init__(self, data_dir: pathlib.Path, epochs=100):
        ...
        def get_spectrogram(waveform):
            ...
            zero_padding = tf.zeros([900] - tf.shape(waveform), dtype=tf.float32)
            ...
            spectrogram = tf.signal.stft(
                equal_length, frame_length=4, frame_step=16) #32,16
            # Obtain the magnitude of the STFT.
            spectrogram = tf.abs(spectrogram)
            # Add a `channels` dimension, so that the spectrogram can be used
            # as image-like input data with convolution layers (which expect
            # shape (`batch_size`, `height`, `width`, `channels`).
            spectrogram = spectrogram[..., tf.newaxis] # spectrogram = tf.abs(spectrogram)
            return spectrogram
        ...
        display.display(display.Audio(waveform, rate=50000000))
